...
EXCLUDES     = ['backupdir','plugin.video.SemperFI','script.module.addon.common','repository.SemperFI','backup','backup.zip']
...
zip = 'special://home/addons/plugin.video.SemperFI'
urlbase      =  'None'
...
USB          =  xbmc.translatePath(os.path.join('special://home/addons/plugin.video.semperFI',''))
...
def BACKUP():  
    ...
    if choice == 0:
	 return
      ...
      exclude_dirs_full =  ['backupdir','plugin.video.semperFI','repository.semperFI','script.skinshortcuts','plugin.program.super.favourites']
...
def UPDATEMENU():
    ...
    if not matchupdate:
	dialog = xbmcgui.Dialog()
        dialog.ok('[COLOR=lime][B]SemperFI
		
		[/B][/COLOR][COLOR=white] Wizard[/COLOR]','NO Updates Currently Available','check back later.','')
	return
